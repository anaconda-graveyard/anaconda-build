...
upload_build_targets(){
    ...
    eval $bb_check_command_error
    {% for tgt in files %}
    echo "anaconda -q -t \$TOKEN upload --user $BINSTAR_OWNER --package $BINSTAR_PACKAGE {{labels}} {{tgt}} --build-id $BINSTAR_BUILD"
    anaconda -q -t "$BINSTAR_API_TOKEN" upload --user "$BINSTAR_OWNER" --package "$BINSTAR_PACKAGE" {{labels}} {{tgt}} --build-id "$BINSTAR_BUILD"
    eval $bb_check_command_error
    {% else %}
