...
:upload_build_targets
    ...
    {% for tgt in files %}
    echo anaconda -q -t %%TOKEN%% upload --user %BINSTAR_OWNER% --package %BINSTAR_PACKAGE% {{labels}} {{tgt}} --build-id %BINSTAR_BUILD_MAJOR%
    anaconda -q -t "%BINSTAR_API_TOKEN%" upload --user "%BINSTAR_OWNER%" --package "%BINSTAR_PACKAGE%" {{labels}} {{tgt}} --build-id "%BINSTAR_BUILD%" || ( {{ set_error() }} )
    {% else %}
